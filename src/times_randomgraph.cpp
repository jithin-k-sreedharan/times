...
void perfect_correct_pairs_DAG(const int& TimeN, const float& pr_alpha,
                               const int& vec_p_1, const int& vec_p_2,
                               const float& pr_beta, const int& vec_q_1,
                               const int& vec_q_2, const float& pr_delta,
                               const float& pr_gamma, const int& no_runs) {
...
void calculate_rho_theta(const int& TimeN, const float& pr_alpha,
                         const int& vec_p_1, const int& vec_p_2,
                         const float& pr_beta, const int& vec_q_1,
                         const int& vec_q_2, const float& pr_delta,
                         const float& pr_gamma, const int& no_runs,
                         const float& p, const int& distance_metric) {
  PNEANet G;
...
void calculate_rho_theta_with_extra_pairs(
    ...
    const int& no_runs, const float& p, const int& distance_metric) {
  PNEANet G;
  ...
  for (int ii = 0; ii < no_runs; ii++) {
    ...
    rank_parallel_len = rank_parallel.Len();
    depth_parallel = depth_parallel + rank_parallel_len;
    // ost1 << theta_extra_temp << "\t" << ((rho_temp +
    // rho_extra_temp)/theta_extra_temp)<< "\n";
...
// 2. rho and theta of guessed pairs, which is perfect pairs + guess of order
// between nodes of different bin+ same bin pairs
// 3. rho and theta of Peeling algorithm
void calculate_rho_theta_improvement(const int& TimeN, const float& pr_alpha,
                                     const int& vec_p_1, const int& vec_p_2,
                                     const float& pr_beta, const int& vec_q_1,
                                     const int& vec_q_2, const float& pr_delta,
                                     const float& pr_gamma, const int& no_runs,
  ...
  uint64_t G_no_nodes;
  float depth_parallel = 0;
  float rho_perf_tt, theta_perf_tt, rho_guess_tt, theta_guess_tt, rho_peel_tt,
      theta_peel_tt, density_guess_tt;
  float rho_perf = 0, theta_perf = 0, rho_guess = 0, theta_guess = 0,
        rho_peel = 0, theta_peel = 0;
  uint64_t rank_parallel_len;
  const bool self_loops_allowed = 1;
  ...
  for (int ii = 0; ii < no_runs; ii++) {
    ...
    // write_dot(G,"original_G.dot.dat","G");
    // write_dot(G_DAG,"DAG_G.dot.dat","DAG(G)");
    std::tie(rho_perf_tt, theta_perf_tt, rho_guess_tt, theta_guess_tt,
             rho_peel_tt, theta_peel_tt) =
        FindRhoThetaPerfectGuess(G, G_DAG, rank_parallel, rank_orig, rank_node,
                                 G_no_nodes);
...
void find_mean_variance_indegree_slbin(
    const int& TimeN, const float& pr_alpha, const int& vec_p_1,
    const int& vec_p_2, const float& pr_beta, const int& vec_q_1,
    const int& vec_q_2, const float& pr_delta, const float& pr_gamma,
    const int& no_runs, const float& p, const int& distance_metric) {
  ...
  for (int ii = 0; ii < no_runs; ii++) {
    ...
    sq_sum = std::inner_product(indeg_sl_bin.begin(), indeg_sl_bin.end(),
                                indeg_sl_bin.begin(), 0.0);
...
void estimate_prob_lemma1_sequential(const int& TimeN, const float& pr_alpha,
                                     const int& vec_p_1, const int& vec_p_2,
                                     const float& pr_beta, const int& vec_q_1,
                                     const int& vec_q_2, const float& pr_delta,
                                     const float& pr_gamma, const int& no_runs,
  ...
  // const bool self_loops_allowed = 1;
  // G = GenPrefAttachGeneral_undirected(TimeN, pr_alpha, vec_p_1, vec_p_2,
  // pr_beta, vec_q_1, vec_q_2, pr_delta, pr_gamma, self_loops_allowed); const
  // char *Desc = "PA Graph generated via Jithin's technique";
  // TSnap::SaveEdgeList(G, FName, Desc);
  // // //----------------------------------
  // // G_no_nodes = (uint64_t)G->GetNodes();
  // // TUInt64UInt64H rank_orig;
  ...
  for (int ii = 0; ii < no_runs; ii++) {
    ...
    if (rank_seq_uni.GetDat(u) < rank_seq_uni.GetDat(v)) emp_average += 1.0;
  }
  emp_average /= no_runs;
...
int main(int argc, char* argv[]) {
  ...
  const int distance_metric =
      Env.GetIfArgPrefixInt("-dm:", 1,
                            "Distance metric; 1:precision and recall, 0:Bin "
                            "based precision (old), 2:Test mode");
  const int choice = Env.GetIfArgPrefixInt(
      "-choice:", 2,
      "0:pr_success_node_0_in_bin_0, 1:count_certain_correct_pairs_DAG, "
      "2:calculate_distance_metric, 3:tree size, 4: tree height test, 5: study "
      "levels, 6: calculate_distance_metric_with_non-probability-1_pairs");
  const bool rgplot =
      Env.GetIfArgPrefixInt("-rgplot:", 0,
                            "0:standard mode, 1:write results for various set "
  ...
  switch (choice) {
    ...
    case 1:  // Compute Recall and Precision of R_{perf} and R_{peel}
      perfect_correct_pairs_DAG(TimeN, pr_alpha, vec_p_1, vec_p_2, pr_beta,
                                vec_q_1, vec_q_2, pr_delta, pr_gamma, no_runs);
      break;

    case 2:  // Compute Recall and Precision of R_{peel}
      calculate_rho_theta(TimeN, pr_alpha, vec_p_1, vec_p_2, pr_beta, vec_q_1,
                          vec_q_2, pr_delta, pr_gamma, no_runs, p,
                          distance_metric);
      break;
    ...
    case 5:  // study number of levels
      ...
      break;
    case 6:  // Compute Recall and Precision using Peeling method. AND predict
             // order in pairs across bins (apart from Probability 1 pairs). AND
             // predict order in pairs inside bins.
      calculate_rho_theta_with_extra_pairs(
          TimeN, pr_alpha, vec_p_1, vec_p_2, pr_beta, vec_q_1, vec_q_2,
          pr_delta, pr_gamma, no_runs, p, distance_metric);
      break;
    case 7:  // Combines theta and rho calculations of perfect pairs, guessed
             // pairs and peeling algorithm
      calculate_rho_theta_improvement(TimeN, pr_alpha, vec_p_1, vec_p_2,
                                      pr_beta, vec_q_1, vec_q_2, pr_delta,
                                      pr_gamma, no_runs, p, distance_metric);
      break;
    case 8:  // Miscellaneous tests
             // find_mean_variance_indegree_slbin(TimeN, pr_alpha, vec_p_1,
             // vec_p_2, pr_beta, vec_q_1, vec_q_2, pr_delta, pr_gamma, no_runs,
             // p, distance_metric);
             // Estimate Probability(\pi^{-1} v < \pi^{-1} w) given by Lemma 1
             // using Sequential technique
      estimate_prob_lemma1_sequential(TimeN, pr_alpha, vec_p_1, vec_p_2,
                                      pr_beta, vec_q_1, vec_q_2, pr_delta,
                                      pr_gamma, no_runs, node_u, node_v);
      break;
    default:
